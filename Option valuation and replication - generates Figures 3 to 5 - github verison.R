...
p <- 1  
# p specifies the number of instances of k simulation. Was 9 in old paper. Can only do 6 here in 24 hrs plus.
# p = 1 for one run (fixed number of time-steps)for Figs 4 & 5; 6 for 6 runs (6 values of time-steps) for Fig 3.
Error_call <- numeric(p)
Error_put <- numeric(p)
Percentage_error_call <- numeric(p)
Percentage_error_put <- numeric(p)
...
# Setting parameters 
  ...
  barrier_type <- "fixed" #"fixed" or "trailing".  NEEDS QUOTES - defines a string to feed to MCVD() function.
  
  S <- 1.0 #stock price at time t
  K <- 1.0 #strike price 
  b <- 0.6 #barrier - for b = K case, set b just slightly less than K, to avoid divisions by zero problems.
  tau <- 5 #time to maturity T - t (in years) 
  r <- 0.015 #risk-free annual interest rate (convenient to set to zero)
  
  mu <- 0.03 #real-world asset drift, used to simulate the payoff we have to replicate
  
  q <- 0.01 #deferment rate (yield) (needs to be slightly different from r, to avoid division-by-zero problems in theta)
  sigma <- 0.20 #annual volatility of the stock price (standard deviation)
  ...
  N <- 60 # 60 * 2^(m-1) # 2400
  # For Fig 4 & 5: N = 2400 with  p = 1 at line 13, to give single run. 
  # For Fig 3: N <- 60 * 2^(m-1), with p = 6 at line 13, to give 60, 120, 240, 480. 960. 1920 runs. 
  
  FD_delta_N_multiplier <- 1 # multiple of N for number of time steps in (longest) FD delta calculations
  
  nSim <- 100 #number of simulations (paths) #)
  FD_delta_nSim_multiplier <- 1  #multipier of nSim for number nSim_temp of simulations in FD delta calcs 
...
#  c(0.04853437, -0.24549251)} # for 10 yrs, b = 0.6, sigma = 0.20, K = 1.0

# if(barrier_type == "fixed"){ Initial_values <- c(0.241158256, -0.198679881) } else {
#                     Initial_values <-   c(0.1473610, -0.4620159)}  # for 10 yrs, b = 0.6, sigma = 0.20, K = 1.25

 if(barrier_type == "fixed") {Initial_values <-  c(0.115606759, -0.21091623)} # for tau = 5, b =0.6, sigma = 0.20 
 if(barrier_type == "trailing") {Initial_values <- c( 0.08074558, -0.37161128)} # for tau = 5, b =0.6, sigma = 0.20 

# b = 0.6, tau = 5, sigma = 0.13   c(0.09138134, -0.3397917) & (0.08484505, -0.39559939)
...
for(j in 2:N){
  ...
  #Bring this next bit into action instead, for fixed barrier
    ...
    # Needed because b_trailing[,j] is the barrier variable used in hedging calcs. Easier to force this to b than change the variable.
    ...
    #I tried vectorisng this loop by adding a 4th dimension to the array for the
    ...
    #browser()
     ...
     for (n in 1:2){
       ...
       # Repeat the nSim rows of X to give starting values for nSim_temp FD simulations. Integer multiple not needed,
       # length.out truncates the last repeat if needed.
        ...
        # b_trailing_temp[,1,,n] <- rep(b_trailing[1:nSim,j] + ifelse(Y[,j] > X[,j],  (n-1) * epsilon, 
        #                                         ifelse(X[,j] - b < epsilon,  (n-1) * epsilon, 0)), length.out = nSim_temp)
       ...
       # We're importing the corresponding b_trailing at step j from the outer loop, to use as the starting value in 
       # the inner loop. . If we're starting with X_temp[,1,,n] very near or below this b_trailing,
       # need to increase the b_trailing_temp (set == b, if "fixed") for n = 2 to correspond with the slightly 
       # higher X_temp. Otherwise, get deep bug: increase in B_temp (and hence B_running-max_temp), as X_temp 
       # progresses further below b_trailing_temp, exactly (to machine precision) offsets the finite difference 
       # in X_temp[n=1&2]. This can lead to Y_temp[1&2] identical. Hence Option_payoff_bought_put-temp[1&2} the same; 
       # their means Thomas_MC_put_price_temp[1&2] the same; and so delta wrongly estimated as 0, even as Y_temp goes some 
       # distance above the barrier. To see the problem, remove the epsilon here and then plot delta for some cases
       # with big positive hedging errors - you'll see the delta gets stuck at 0 after the first barrier touch.'
     ...
     Thomas_delta_bought_put_by_FD[,j] <- pmin(pmax(((Thomas_MC_put_price_temp[,j,2] - Thomas_MC_put_price_temp[,j,1])/epsilon) * X[,j]/Y[,j] , -1) ,0)  
         ...
         # if(barrier_type =="trailing"){
         #     Thomas_delta_bought_put_by_FD[,j] <- ifelse (abs(Y[,j] - b_trailing[,j]) == 0, 0, Thomas_delta_bought_put_by_FD[,j])
         #   }else{
         #     Thomas_delta_bought_put_by_FD[,j] <- ifelse (abs(Y[,j] - b_as_matrix[,j]) == 0, 0, Thomas_delta_bought_put_by_FD[,j])
         #   }
     ...
     # pmin,pmax restricts delta to (-1,0) to discard bad FD estimaates
     # Then set delta to 0 if Y is exactly at the barrier? - NO, causes jerky estimates near barrier.  
...
# THIS NEEDS subset [1] to refer to [m] when reinstate the m-loop
...
#Monte_Carlo_put_price <- Thomas_MC_put_price_temp[1,1,1] 
# that is now the impprted figure from MCVD()
# Could alternately use expected_payoff_put[,2,], and then it references nSim x (1 for step 2) x xnSim_temp dimensions in k loop here
# That's a trick for quickiy getting lots of sims. The "missing" step 1 is on average exp(((tau/N)* r).

#dS_T_by_dS_0 <- X[,N]* exp(pmax ( log(b) - log(apply(X,1,min)), 0) ) / X[,1]
#Pathwise_delta_bought_put <- - exp(-r*tau) * mean(ifelse(payoff_expiry_put >0, dS_T_by_dS_0,0))

#payoff_expiry_put_on_X <- pmax(K-X[,N],0)
#expected_payoff_put_on_X <- mean(payoff_expiry_put_on_X)
#Monte_Carlo_put_on_X_price <- exp(-r*(tau))*expected_payoff_put_on_X
#Pathwise_delta_bought_put_on_X <- - exp(-r*tau) * mean(ifelse(payoff_expiry_put_on_X >0, X[,N]/S,0))
...
Thomas_st_dev_hedging_error_graph  <- rbind(Thomas_st_dev_hedging_error,
...
Thomas_st_dev_hedging_error_analytic_graph  <- rbind(Thomas_st_dev_hedging_error_analytic,
...
Thomas_call_st_dev_hedging_error_graph  <- rbind(Thomas_call_st_dev_hedging_error,
                                     c(log(N),log(Thomas_call_st_dev_hedging_error)))


#Ratio_call <- Monte_Carlo_call_price[m-1]/Analytic_barrier_call
#Ratio_put <- Monte_Carlo_put_price[m-1]/Analytic_barrier_put

Error_call[m] <- Monte_Carlo_call_price - Analytic_barrier_call
Error_put[m] <- Monte_Carlo_put_price - Analytic_barrier_put

Percentage_error_call[m] <- (Error_call[m]/Analytic_barrier_call) * 100
Percentage_error_put[m] <- (Error_put[m]/Analytic_barrier_put) * 100
...
#hist(BS_hedging_error, xlab="", ylim=c(0, nSim/3), breaks = mybins2)
...
#cat("\nSt Dv Thms hg err: Call (fxd b, analytic):",round(Thomas_call_st_dev_hedging_error,digits=7), "Put ditto:"
#    ,round(Thomas_st_dev_hedging_error_analytic,digits=7))
#cat("\nPut ditto (trailing b, Monte Carlo & FD):",round(Thomas_st_dev_hedging_error,digits=7))

#cat("\nRolled-up difference in call valuations: Analytic (C - C') ):",   Rolled_up_difference_in_call_valuations)
